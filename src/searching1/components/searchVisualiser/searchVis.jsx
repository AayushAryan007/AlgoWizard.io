...
class ArrayVisualier extends Component {
  ...
  randomSearchNum = () => {
    let numToSearch = Math.floor(
      Math.random() * (this.state.numberOfElements - 1 + 1) + 1
    );
  ...
  binarySearchAlgo = () => {
    ...
    const animations = getBinarySearchAnimations(
      this.state.arrayOfNums,
      this.state.numberToFind
    );
    ...
    for (let i = 0; i < animations.length; i++) {
      ...
      if (isColorChange) {
        ...
        setTimeout(() => {
          ...
          const filtredDivs = Array.from(divs).filter(
            (x) => !divsNotToInclude.includes(x)
          );
        ...
        if (i !== animations.length - 1) {
          ...
          setTimeout(() => {
            alert(
              `Success! The target number: ${this.state.numberToFind} is found!`
            );
            middle_div_style.backgroundColor = "white";
            middle_div_style.border = `3px solid ${HIGHLIGHT_COLOR}`;
        ...
        <div className="text-center  text-black font-bold">
          <div className="row">
            <div className="col">
              <div className="sliderDesc">
                Size of the sorted array to search:{" "}
            ...
            <div className="col">
              Target number to look in the array:{" "}
              <div className="array-box-unique  text-black font-bold" key="50">
                {this.state.numberToFind}
              </div>
              <div>
                <button
          ...
          <div className="row">
            <div className="col">
              <div className="colorPalet">
                <ul>
                  <li>
                    <div className="array-box-color">0</div>Color of the
                    Boundaries Selected
                  </li>
                  <li>
                    <div className="array-box-color2">0</div>Color of the
                    Selected Number to Compare with Target Number
                  </li>
                </ul>
              </div>
            </div>
          ...
          <div className="array-container">
            {this.state.arrayOfNums.map((num, idx) => (
              <div className="array-box text-black font-bold" key={idx}>
                {num}
              </div>
            ))}
          </div>
